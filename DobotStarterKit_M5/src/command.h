...
typedef struct tagARCCmd {
 ...
 float z;
 float r;
 }cirPoint; //圆弧中间点，需设置为笛卡尔坐标
 struct {
 float x;
 float y;
 float z;
 float r;
 }toPoint; //圆弧目标点，需设置为笛卡尔坐标
}ARCCmd;
...
** ARC function
*********************************************************************************************************/
extern int SetARCCmd(ARCCmd *arcCmd, bool isQueued, uint64_t *queuedCmdIndex);
extern float GetPose_r(void);
extern Pose GetPose(void);
